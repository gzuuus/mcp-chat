...
async function testToolsWithoutAPI() {
  ...
  try {
    const result = await calculatorTool.execute({ operation: 'add', a: 15, b: 27 });
  ...
  try {
    const result = await weatherTool.execute({ city: 'San Francisco', units: 'celsius' });
  ...
  try {
    const result = await timeTool.execute({ timezone: 'America/New_York', format: '12h' });
  ...
  try {
    const result = await textTool.execute({ text: 'Hello World!', operation: 'reverse' });
...
async function testToolsWithAPI() {
  ...
  try {
    ...
    const configWithTools: AssistantConfig = {
      ...
      system: 'You are a helpful assistant with access to various tools. Use the available tools to help answer questions and perform tasks. Be clear about what tools you are using and explain the results.'
    };
    ...
    for (const query of testQueries) {
      ...
      try {
        ...
        tui.showError(`Error: ${error instanceof Error ? error.message : String(error)}`);
      }
...
async function main() {
  ...
  console.log(`${apiTestSuccessful ? '✅' : '⚠️ '} API integration: ${apiTestSuccessful ? 'Working' : 'Requires API key'}`);
  
  if (!apiTestSuccessful) {
    console.log('\n💡 To test the full functionality:');
    console.log('1. Get an OpenAI API key from https://platform.openai.com/account/api-keys');
